				...
				<div class="ibox-title clearfix">
					...
					<div class="ibox-tools">
						<shiro:hasPermission name="仓库添加">
							<a
								href="${pageContext.request.contextPath}/admin/storehouse/name/add"
						...
						<tbody>
							<c:forEach var="storehouse" items="${storehouses}" varStatus="i">
								<tr>
									...
									<td>${storehouse.storehouseMin}</td>
									<td>${storehouse.storehouseMax}</td>
									<td>
										<div class="col-sm-12 h-100 p-lg">
											<shiro:hasPermission name="仓库更新">
												<a
													href="${pageContext.request.contextPath}/admin/storehouse/name/update?pkSid=${storehouse.pkSid}"
													class="btn btn-primary btn-sm ">修改</a>
											</shiro:hasPermission>
											<shiro:hasPermission name="仓库删除">
												<button class="btn btn-danger btn-sm demo4 "
													data="${storehouse.pkSid}">删除</button>
											</shiro:hasPermission>
										</div>
									</td>
												...
												swal(
														...
														function(isConfirm) {
															if (isConfirm) {
																...
																$
																		.ajax({
																			url : "${pageContext.request.contextPath}/admin/storehouse/name/delete?pkSid="
																					+ $(
																							e.target)
																							.attr(
																									'data'),
																				...
																				if (d.tips == "true") {
																					swal(
																							...
																							function(
																									isConfirm) {
																								if (isConfirm) {
																									window.location.href = '${pageContext.request.contextPath}/admin/storehouse/name/show';
																								} else {
