						...
						<tbody>
							<c:forEach var="product" items="${products}" varStatus="i">
								<tr>
									...
									<td>
										${firstCategorysNameIndex[secondCategorysIndex[thirdCategorysIndex[product.pkPcid]]]}-
										${secondCategorysNameIndex[thirdCategorysIndex[product.pkPcid]]}-
										${thirdCategorysNameIndex[product.pkPcid]}</td>
									<td>${product.productCategoryNum}</td>
									<td>${product.productCategoryPrice/100}</td>
									<td>${product.vipPrice/100}</td>
									<td>
										<div class="col-sm-12 h-100 p-lg">
											<shiro:hasPermission name="采购管理">
												<a
													href="${pageContext.request.contextPath}/admin/product/buy/add?pkPid=${product.pkPid}"
													class="btn btn-warning btn-sm demo3">采购</a>
											</shiro:hasPermission>
											<shiro:hasPermission name="货物盘点修改">
												<a
													href="${pageContext.request.contextPath}/admin/product/update?pkPid=${product.pkPid}"
													class="btn btn-primary btn-sm ">修改</a>
											</shiro:hasPermission>
											<shiro:hasPermission name="货物盘点删除">
												<button class="btn btn-danger btn-sm demo4 "
													data="${product.pkPid}">删除</button>
											</shiro:hasPermission>
										</div>
									</td>
													...
													swal(
															...
															function(isConfirm) {
																if (isConfirm) {
																	...
																	$
																			.ajax({
																				url : "${pageContext.request.contextPath}/admin/product/delete?pkPid="
																						+ $(
																								e.target)
																								.attr(
																										'data'),
																					...
																					if (d.tips == "true") {
																						swal(
																								...
																								function(
																										isConfirm) {
																									if (isConfirm) {
																										window.location.href = '${pageContext.request.contextPath}/admin/product/show';
																									} else {
