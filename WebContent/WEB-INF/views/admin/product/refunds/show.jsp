...
<%-- 						<a href="${pageContext.request.contextPath}/admin/product/refunds/info/add" class="btn btn-success btn-sm">添加</a> --%>
					</div>
				</div>
				...
				<table class="table table-bordered table-hover ">
						...
						<tbody>
						<c:forEach var="refund" items="${refunds}" varStatus="i">
								<tr>

									<td>${i.count}</td>
									<td>${refund.refundsTime}</td>
									<td>${employeesIndex[refund.pkUid]}</td>
									<td>${refund.refundsResult}</td>
									<td>
										<div class="col-sm-12 h-100 p-lg">
<%-- 											<shiro:hasPermission name="货物盘点修改"> --%>
												<a
													href="${pageContext.request.contextPath}/admin/product/refunds/info/show?pkReid=${refund.pkReid}"
													class="btn btn-primary btn-sm ">查看</a>
<%-- 											</shiro:hasPermission> --%>
<%-- 											<shiro:hasPermission name="货物盘点删除"> --%>
												<button class="btn btn-danger btn-sm demo4 "
													data="${refund.pkReid}">删除</button>
<%-- 											</shiro:hasPermission> --%>
										</div>
									</td>
										...
										swal(
												...
												function(isConfirm) {
													if (isConfirm) {
														...
														$
																.ajax({
																	url : "${pageContext.request.contextPath}/admin/product/sales/delete?pkReid="
																			+ $(
																					e.target)
																					.attr(
																							'data'),
																		...
																		if (d.tips == "true") {
																			swal(
																					...
																					function(
																							isConfirm) {
																						if (isConfirm) {
																							window.location.href = '${pageContext.request.contextPath}/admin/product/refunds/info/show?pkReid=${refund.pkReid}';
																						} else {
