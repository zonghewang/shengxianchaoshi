			...
			<div class="ibox float-e-margins">
				<div class="ibox-title clearfix">
					...
					<div class="ibox-tools">
						<a href="${pageContext.request.contextPath}/admin/product/buy/buyandinfo/add" class="btn btn-success btn-sm">添加</a>
					</div>
				</div>
				...
				<table class="table table-bordered table-hover ">
						...
						<tbody>
						<c:forEach var="buy" items="${buys}" varStatus="i">
								<tr>

									<td>${i.count}</td>
									<td>${buy.buyName}</td>
									<td>${buy.buyTime}</td>
									<td>${employeesIndex[buy.pkUid]}</td>
									<td>${buy.buyInfo}</td>
									<td>
										<div class="col-sm-12 h-100 p-lg">
<%-- 											<shiro:hasPermission name="货物盘点修改"> --%>
												<a
													href="${pageContext.request.contextPath}/admin/product/buy/buyandinfo/show?pkBid=${buy.pkBid}"
													class="btn btn-primary btn-sm ">查看</a>
<%-- 											</shiro:hasPermission> --%>
<%-- 											<shiro:hasPermission name="货物盘点删除"> --%>
												<button class="btn btn-danger btn-sm demo4 "
													data="${buy.pkBid}">删除</button>
<%-- 											</shiro:hasPermission> --%>
										</div>
									</td>
										...
										swal(
												...
												function(isConfirm) {
													if (isConfirm) {
														...
														$
																.ajax({
																	url : "${pageContext.request.contextPath}/admin/product/buy/delete?pkBid="
																			+ $(
																					e.target)
																					.attr(
																							'data'),
																		...
																		if (d.tips == "true") {
																			swal(
																					...
																					function(
																							isConfirm) {
																						if (isConfirm) {
																							window.location.href = '${pageContext.request.contextPath}/admin/product/buy/show';
																						} else {
