...
<shiro:hasPermission name="三级分类添加">
<!DOCTYPE html>
<html>
		...
		$('#firstselect')
				.click(
						function() {
							...
							$
									.ajax({
										url : "${pageContext.request.contextPath}/admin/productcategory/thread/add?getdata="
												+ $(this).val(),
										type : "POST",
										success : function(d) {
											$('#secondselect').empty();
		...
		.ready(
				function() {
					$(".demo2")
							.click(function() {
										$.ajax({
											url: "${pageContext.request.contextPath}/admin/productcategory/thread/add",
											type:"POST",
											data:$("form").serialize(),
											success: function(d){
												if(d.tips=="false"){
													...
													swal(
															...
															function(isConfirm) {
																if (isConfirm) {
																	window.location.href=ctxpath+'/admin/productcategory/thread/show';
																} else {
